...
namespace Revital.Supply
{
    ...
    [Ui("商户进货", "cart")]
    public class BizlyUpWork : WebWork
    {
        protected override void OnMake()
        {
            MakeVarWork<BizlyUpVarWork>();
        }
        ...
        public async Task @default(WebContext wc, int page)
        {
            ...
            dc.Sql("SELECT ").collst(Up.Empty).T(" FROM ups WHERE partyid = @1 AND status = 0 ORDER BY id");
            var arr = await dc.QueryAsync<Up>(p => p.Set(orgid));
        ...
        public async Task buys(WebContext wc, int page)
        {
            short orgid = wc[-1];
            using var dc = NewDbContext();
            dc.Sql("SELECT ").collst(Up.Empty).T(" FROM ups WHERE partyid = @1 AND status >= ").T(Up.STATUS_SUBMITTED).T(" ORDER BY id");
            var arr = await dc.QueryAsync<Up>(p => p.Set(orgid));
        ...
        public async Task @new(WebContext wc)
        {
            ...
            {
                var o = await wc.ReadObjectAsync<Up>(0);
                using var dc = NewDbContext();
                dc.Sql("INSERT INTO buys ").colset(Up.Empty, 0)._VALUES_(Up.Empty, 0);
    ...
    [Ui("销售分拣管理", "sign-out")]
    public class CtrlyUpWork : WebWork
    {
        protected override void OnMake()
        {
