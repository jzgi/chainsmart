...
public class SuplyLotVarWork : LotVarWork
{
    ...
    public async Task edit(WebContext wc)
    {
        ...
        if (wc.IsGet)
        {
            ...
            wc.GivePane(200, h =>
            {
                ...
                h.LI_().SELECT("消息类型", nameof(o.typ), o.typ, Lot.Typs)._LI();
        ...
        else // POST
        {
            ...
            var m = await wc.ReadObjectAsync(msk, new Lot
            {
            ...
            // update
            using var dc = NewDbContext();
            dc.Sql("UPDATE lotops ")._SET_(Lot.Empty, msk).T(" WHERE id = @1 AND orgid = @2");
    ...
    public async Task ok(WebContext wc)
    {
        ...
        using var dc = NewDbContext();
        dc.Sql("UPDATE lots SET status = 4, oked = @1, oker = @2 WHERE id = @3 AND orgid = @4 RETURNING ").collst(Lot.Empty);
        var o = await dc.QueryTopAsync<Lot>(p => p.Set(DateTime.Now).Set(prin.name).Set(id).Set(org.id));
