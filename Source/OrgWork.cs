...
public abstract class OrgWork<V> : WebWork where V : OrgVarWork, new()
{
    ...
    protected static void MainGrid(HtmlBuilder h, IEnumerable<Org> lst, User prin, bool mktly)
    {
        h.MAINGRID(lst, o =>
        {
            ...
            if (o.icon)
            {
                h.PIC(MainApp.WwwUrl, "/org/", o.id, "/icon", css: "uk-width-1-6");
            }
            else
            {
...
public class AdmlyEstWork : OrgWork<AdmlyEstVarWork>
{
    ...
    public void @default(WebContext wc, int page)
    {
        ...
        var arr = GrabTwinArray<int, Org>(0, filter: x => x.IsRtlEst, sorter: (x, y) => x.regid - y.regid).GetSegment(20 * page, 20);
    ...
    public async Task @new(WebContext wc, int cmd)
    {
        ...
        if (wc.IsGet)
        {
            ...
            wc.GivePane(200, h =>
            {
                h.FORM_().FIELDSUL_(o.IsRtlEst ? "新建市场机构" : "新建供应机构");
