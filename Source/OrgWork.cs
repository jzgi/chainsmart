    ...
    public async Task @new(WebContext wc, int cmd)
    {
        ...
        if (wc.IsGet)
        {
            ...
            wc.GivePane(200, h =>
            {
                ...
                h.LI_().SELECT("类型", nameof(o.typ), o.typ, cmd == 1 ? Org.MktTyps : Org.CtrTyps, required: true)._LI();
                ...
                if (cmd == 1)
                {
                    h.LI_().SELECT("关联云仓", nameof(o.hubid), o.hubid, ctrs, required: true)._LI();
                }
...
public class MktlyOrgWork : OrgWork<MktlyOrgVarWork>
{
    ...
    public void @default(WebContext wc, int page)
    {
        var org = wc[-1].As<Org>();
        var prin = (User)wc.Principal;
        var array = GrabTwinArray<int, Org>(org.id, filter: x => x.AsRtl && x.status == 4, sorter: (x, y) => x.addr.CompareWith(y.addr));
        var arr = array.Segment(20 * page, 20);
    ...
    public void down(WebContext wc, int page)
    {
        var org = wc[-1].As<Org>();
        var prin = (User)wc.Principal;
        var array = GrabTwinArray<int, Org>(org.id, filter: x => x.AsRtl && x.status is 1 or 2, sorter: (x, y) => x.addr.CompareWith(y.addr));
        var arr = array.Segment(20 * page, 20);
    ...
    public void @void(WebContext wc, int page)
    {
        var org = wc[-1].As<Org>();
        var prin = (User)wc.Principal;
        var array = GrabTwinArray<int, Org>(org.id, filter: x => x.AsRtl && x.status == 0, sorter: (x, y) => x.addr.CompareWith(y.addr));
        var arr = array.Segment(20 * page, 20);
...
public class CtrlySupWork : OrgWork<CtrlySupVarWork>
{
    ...
    public void @default(WebContext wc, int page)
    {
        ...
        var array = GrabTwinArray<int, Org>(org.id, filter: x => x.status == 4 && x.IsSup, sorter: (x, y) => x.oked.CompareTo(y.oked));
        var arr = array.Segment(20 * page, 20);
...
public class CtrlySrcWork : OrgWork<CtrlySrcVarWork>
{
    ...
    public void @default(WebContext wc, int page)
    {
        ...
        var array = GrabTwinArray<int, Org>(org.id, filter: x => x.status == 4 && x.IsSrc, sorter: (x, y) => x.oked.CompareTo(y.oked));
        var arr = array.Segment(20 * page, 20);
    ...
    public async Task @new(WebContext wc)
    {
        ...
        if (wc.IsGet)
        {
            wc.GivePane(200, h =>
            {
                ...
                h.LI_().TEXT("产源名", nameof(o.name), o.name, max: 12, required: true)._LI();
                h.LI_().TEXTAREA("简介语", nameof(o.tip), o.tip, max: 40)._LI();
                h.LI_().TEXT("工商登记名", nameof(o.legal), o.legal, max: 20, required: true)._LI();
                h.LI_().SELECT("省份", nameof(o.regid), o.regid, regs, filter: (_, v) => v.IsProvince, required: true)._LI();
                h.LI_().TEXT("联系地址", nameof(o.addr), o.addr, max: 30)._LI();
