...
public abstract class AdVarWork : WebWork
{
    public async Task @default(WebContext wc)
    {
        ...
        dc.Sql("SELECT ").collst(Ad.Empty).T(" FROM tags WHERE id = @1 AND parentid = @2");
        var o = await dc.QueryTopAsync<Ad>(p => p.Set(id).Set(org.id));
...
public class MktlyAdVarWork : AdVarWork
{
    ...
    public async Task edit(WebContext wc)
    {
        ...
        {
            using var dc = NewDbContext();
            dc.Sql("SELECT ").collst(Ad.Empty).T(" FROM tags WHERE id = @1");
            ...
            wc.GivePane(200, h =>
            {
                ...
                h.LI_().SELECT("民生卡类型", nameof(o.typ), o.typ, Ad.Typs, filter: (k, _) => k <= 2, required: true)._LI();
