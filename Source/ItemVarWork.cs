    ...
    public class ShplyItemVarWork : ItemVarWork
    {
        ...
        public async Task edit(WebContext wc)
        {
            ...
            else // POST
            {
                ...
                // update
                using var dc = NewDbContext();
                dc.Sql("UPDATE items ")._SET_(Item.Empty, msk).T(" WHERE id = @1 AND shpid = @2");
        ...
        public async Task stock(WebContext wc)
        {
            ...
            else // POST
            {
                ...
                {
                    dc.Sql("UPDATE items SET ops = (CASE WHEN ops[16] IS NULL THEN ops ELSE ops[2:] END) || ROW(@1, @2, @3, (avail + @3), @4)::StockOp, avail = avail + @3, stock = stock + @3 WHERE id = @5 AND shpid = @6");
