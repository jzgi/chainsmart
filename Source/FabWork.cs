...
public class SuplyFabWork : FabWork<SuplyFabVarWork>
{
    ...
    public void @default(WebContext wc)
    {
        ...
        var arr = GrabTwinArray<Fab>(org.id, filter: x => x.status == 4, comp: (x, y) => x.oked.CompareTo(y.oked));
    ...
    public void down(WebContext wc)
    {
        ...
        var arr = GrabTwinArray<Fab>(org.id, filter: x => x.status is 1 or 2, comp: (x, y) => x.oked.CompareTo(y.oked));
    ...
    public void @void(WebContext wc)
    {
        ...
        var arr = GrabTwinArray<Fab>(org.id, filter: x => x.status == 0, comp: (x, y) => x.adapted.CompareTo(y.adapted));
