    ...
    public class AdmlyProdVarWork : WebWork
    {
        ...
        public async Task upd(WebContext wc)
        {
            ...
            if (wc.IsGet)
            {
                ...
                dc.Sql("SELECT ").collst(Prod.Empty).T(" FROM mats WHERE id = @1");
                var o = dc.QueryTop<Prod>(p => p.Set(id));
                wc.GivePane(200, h =>
                {
                    h.FORM_().FIELDSUL_("填写资料");
                    h.LI_().SELECT("食材类型", nameof(o.typ), o.typ, Prod.Typs)._LI();
            ...
            else // POST
            {
                ...
                using var dc = NewDbContext();
                dc.Sql("UPDATE mats")._SET_(Prod.Empty, 0).T(" WHERE id = @1");
                dc.Execute(p =>
                {
                    o.Write(p, 0);
                    p.Set(id);
