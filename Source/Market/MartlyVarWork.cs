...
{
    [Ui("账号")]
    public class MartlyVarWork : WebWork
    {
        protected override void OnMake()
        {
            MakeWork<OrglyAccessWork>("acc", User.Mrtly);

            MakeWork<MrtlyBizWork>("biz");

            MakeWork<MrtlyUserWork>("user");

            MakeWork<MartlyBoardWork>("bd");
        }
        ...
        public async Task @default(WebContext wc)
        {
            ...
            if (!inner)
            {
                wc.GiveFrame(200, false, 60, title: "市场管理", group: (byte) o.typ);
            }
            else
            {
