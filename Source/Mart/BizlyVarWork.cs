...
namespace Zhnt.Mart
{
    ...
    public class BizlyVarWork : WebWork
    {
        ...
        {
            MakeWork<BizlyAccessWork>("acc");

            // mart

            MakeWork<BizlyBizWork>("biz");
        ...
        public async Task @default(WebContext wc)
        {
            ...
            short id = wc[0];
            var bizs = Fetch<Map<short, Biz>>();
            var o = bizs[id];
            if (!inner)
            {
                wc.GiveFrame(200, false, 60, title: "经营方操作", group: (byte) o.typ);
            }
            else
            {
        ...
        {
            short orgid = wc[0];
            var obj = Fetch<Map<short, Biz>>()[orgid];
            if (wc.IsGet)
            {
                using var dc = NewDbContext();
