...
namespace Zhnt.Mart
{
    ...
    [Ui("材料")]
    public class AdmlyMatWork : WebWork
    {
        protected override void OnMake()
        {
            MakeVarWork<AdmlyMatVarWork>();
        }

        public void @default(WebContext wc, int page)
        {
            using var dc = NewDbContext();
            dc.Sql("SELECT ").collst(Mat.Empty).T(" FROM mats ORDER BY typ, status DESC, id LIMIT 40 OFFSET 40 * @1");
            var arr = dc.Query<Mat>(p => p.Set(page));
            wc.GivePage(200, h =>
            {
                h.TOOLBAR();
                h.TABLE_();
                ...
                foreach (var o in arr)
                {
                    if (o.typ != last)
                    {
                        h.TR_().TD_("uk-label uk-padding-tiny-left", colspan: 4).T(Mat.Typs[o.typ])._TD()._TR();
                    }
                    h.TR_();
                    h.TDCHECK(o.id);
                    h.TD_().VARTOOL(o.Key, nameof(AdmlyMatVarWork.upd), caption: o.name).SP().SUB(o.unit)._TD();
        ...
        public async Task @new(WebContext wc)
        {
            if (wc.IsGet)
            {
                ...
                wc.GivePane(200, h =>
                {
                    ...
                    h.LI_().SELECT("材料类型", nameof(o.typ), o.typ, Mat.Typs)._LI();
            ...
            else // POST
            {
                ...
                using var dc = NewDbContext();
                dc.Sql("INSERT INTO mats ").colset(Mat.Empty, 0)._VALUES_(Mat.Empty, 0);
