        ...
        peerConn.setLocalDescription(offer).then(function () {
            ...
            xhr.onreadystatechange = function () {
                ...
                if (4 == this.readyState) {
                    if (200 == this.status || 201 == this.status) { // ok or created
                        var jo = JSON.parse(this.responseText);
                        peerConn.setRemoteDescription(jo);
                    } else { 
                        alert('the signaling server returns error');
                    }
                }
            };
            xhr.open(method, 'http://' + signalSvr + '/rtc', false);
...
function buyRecalc(trig) {
    ...
    if (frm.fee) {
        ...
        if (frm.area) { 
            ...
            frm.addr.maxLength = (frm.area.value == '外地') ? 40: 15;
